...
| `runBox.localWorkspaceFilesPath`   | `string`  | `""`         | Relative path from the workspace root for extension-local files (variables, favorites, etc.). When empty, files are stored in `.vscode/`. Example: `.temp` or `.temp/runbox`. Use forward slashes (`/`) as separators; backslashes also work but are normalized to forward slashes internally so the path resolves correctly on every OS. Changing this setting does not move existing files, see note below. |
| `runBox.workspaceID`               | `string`  | `""`         | Auto-generated unique identifier that links the **Current Workspace** pseudo-category to this specific workspace folder. Created automatically the first time you add a group or command under **Current Workspace** — you never need to set it manually. Do not copy this value between projects.                                                                                                            |
