...
| `writeWorkspaceCommandsSection(input, fsPath?)` | fn    | Writes the "workspaceCommands" section via read-modify-write, preserving "variables"/"favorites". Calls `ensureWorkspaceId` first and stamps the resulting ID into the section. Returns `false` if no workspace folder is open.                                                                                                                                                                                                  |
...
| `handlePickFile(panel, payload)`                             | `pickFile`                  | Shows a native single-file `showOpenDialog`; posts `pickFileResult` with the selected `fsPath` (or nothing if cancelled)                                                                                                                                                                                                                     |
...
| `handleSaveWorkspaceCommandsData(panel, payload, postState)` | `saveWorkspaceCommandsData` | Persists the "Current Workspace" pseudo-category groups/commands via `writeWorkspaceCommandsSection`; posts `saveWorkspaceCommandsDataResult`                                                                                                                                                                                                |
| `handleSaveCommandMove(panel, payload, postState)`           | `saveCommandMove`           | Atomically persists a command move between a regular category (`commands.json`) and "Current Workspace" (`runbox.data.json`) — writes both files sequentially, calls `postState` once; posts `saveResult`. See "Current Workspace" section below for why this must be one message, not two.                                                  |
...
- **Add/Edit Command form:** `commandFormBuffer.categoryId` can hold `CURRENT_WORKSPACE_CATEGORY_ID`. `submitEditCommand()` supports **moving** a command between "Current Workspace" and a regular category — it removes the command from its old array and pushes it into the new one, then persists both sides **atomically in one `saveCommandMove` message** via `persistCommandMoveThenRender()` (`media/utils.js`). This must not be split into two separate `saveData` + `saveWorkspaceCommandsData` messages: VS Code's `onDidReceiveMessage` does not serialize async handler execution, so two independent handlers (each calling `postState()` internally) can interleave — whichever `postState()` resolves last reads the _other_ file before it finished writing, making the moved command appear in neither source (looks deleted). Moving a command out of a regular category into "Current Workspace" also strips it from Global favorites (see below) via a separate, unrelated `saveFavorites` message — that one is safe to send independently since it does not touch the same two files.
...
| 9          | `media/modals/ai-explain.js`      | AI Explain modal — opens immediately in loading state when `.btn-explain` is clicked; displays structured Markdown explanation of a CLI command returned by `explainWithAI`. Manages `aiExplainState`.                                                                                                                                                                                                                                                                                                                                         | `openAiExplainModal`, `closeAiExplainModal`, `handleAiExplainResult`                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                 |
| 10         | `media/modals/enum-manager.js`    | Enum Manager modal - manages enum variable options (title/value/description). Opened from the command form and writes through `commandFormBuffer.variableMeta`                                                                                                                                                                                                                                                                                                                                                                                 | `renderEnumManagerModal`, `bindEnumManagerEvents`                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                    |
| 11         | `media/modals/ai-check-status.js` | AI Check Status modal — shared by "Check Connection" and "Check Rate Limits" buttons in AI Settings. Injected directly into `document.body` (same pattern as `ai-explain.js`), independent of the main `render()` cycle. Shows a loading spinner, then either the connection result, the rate limit values (generic label/value rows built from whatever keys the provider returned), or — for providers with `hasApiRateLimits: false` — a message plus a clickable link to the provider's own rate-limit page. Manages `aiCheckStatusState`. | `openAiCheckConnectionModal`, `openAiCheckRateLimitsModal`, `closeAiCheckStatusModal`, `handleAiCheckConnectionResult`, `handleAiCheckRateLimitsResult`                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                              |
