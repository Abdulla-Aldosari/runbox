 ...
 * Persists a command move between a regular category (global commands.json) and the
 * "Current Workspace" pseudo-category (workspace-local runbox.data.json) atomically:
 * both files are written sequentially within this single message round trip, then
 * postState() is called exactly once at the end.
 *
 * This avoids a race condition present when the two writes are sent as separate
 * messages (saveData + saveWorkspaceCommandsData): VS Code's onDidReceiveMessage does
 * not serialize async handler execution, so the two handlers' internal awaits (file
 * reads/writes, postState's own disk reads) can interleave. Whichever handler's
 * postState() call happens to resolve last "wins" as the final state sent to the
 * webview — if it reads the other file before that other handler finished writing it,
 * the moved command would appear in neither source and look deleted.
 *
 * @param {import('vscode').WebviewPanel} panel
 * @param {{ data: object, workspaceCommands: { groups: object[], commands: object[] }, activeFsPath: string|null }} payload
 * @param {function} postState - Callback to refresh webview state
 */
async function handleSaveCommandMove(panel, payload, postState) {
  try {
    const activeFsPath =
      payload && typeof payload.activeFsPath === "string" && payload.activeFsPath ? payload.activeFsPath : null;
    const workspaceCommands = (payload && payload.workspaceCommands) || {};

    const normalizedData = normalizeCommandsData(payload && payload.data);
    await writeCommandsData(normalizedData);

    const wasCreated = await writeWorkspaceCommandsSection(
      { groups: workspaceCommands.groups, commands: workspaceCommands.commands },
      activeFsPath
    );
    if (wasCreated) {
      notifyLocalDataFileCreated();
